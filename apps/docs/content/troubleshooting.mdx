...
STRIPE_API_KEY=YOUR_STRIPE_TEST_MODE_KEY
STRIPE_WEBHOOK_SECRET=YOUR_STRIPE_TEST_MODE_WEBHOOK_SECRET
...
> **Note:** `STRIPE_API_KEY` is the API v2 equivalent of `STRIPE_PRIVATE_KEY` used by the web app. Both are Stripe secret keys but are consumed by different services. If you don't need Stripe functionality in the API, use a noncredential placeholder, but the variable must be present.
