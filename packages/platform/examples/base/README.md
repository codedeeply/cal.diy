...
2. Open "apps/api/v2/.env" and copy environment variables below, and then copy `NEXTAUTH_SECRET` from the root ".env" of repository cloned in step 1. Generate a unique `JWT_SECRET` locally with `node -e 'console.log(require("node:crypto").randomBytes(32).toString("hex"))'` and replace the placeholder in your local `.env`. Do not commit or share the generated value.
```jsx
...
JWT_SECRET="<GENERATE_A_UNIQUE_LOCAL_SECRET>"
